...
def build_system(profile, onboarding_mode=False, uid=None, cal_events=None):
    ...
    if uid and get_google_token(uid):
        cal_block = (
            ...
            "НИКОГДА не предлагай пользователю подключить календарь — он уже подключён. "
            "Ты управляешь календарём через специальные теги в конце ответа.\n\n"
            "КАК УПРАВЛЯТЬ:\n"
            "ДОБАВИТЬ → [TASK: название | приоритет | сфера | timeframe | HH:MM]\n"
            "УДАЛИТЬ ВСЕ → [CAL_DELETE_ALL]\n"
            "УДАЛИТЬ ОДНО → [CAL_DELETE: event_id]\n"
            "ИЗМЕНИТЬ → [CAL_UPDATE: event_id | новое название | YYYY-MM-DD | HH:MM]\n"
            "УДАЛИТЬ ПАПКУ → [CAL_DELETE_CALENDAR: calendar_id]\n\n"
            "Примеры использования:\n"
            '- "добавь задачу" → [TASK: ...]\n'
            '- "очисти весь календарь" → [CAL_DELETE_ALL]\n'
            '- "удали событие X" → [CAL_DELETE: id из списка ниже]\n'
            '- "перенеси встречу" → [CAL_UPDATE: id | ...]\n'
            '- "удали папку/доп.календарь" → [CAL_DELETE_CALENDAR: id]\n\n'
            + cal_events_block
        )
    else:
